...
def main():
    try:
        ...
        if start_idx != -1 and end_idx != -1:
            print(f"Memblok teks dari paragraf {start_idx + 2} sampai {end_idx - 1}")
            start_pos = paragraphs(start_idx + 2).Range.Start
            end_pos = paragraphs(end_idx - 1).Range.End
            
            # Use Selection to delete, which simulates user pressing Delete key
            # This handles complex objects much better than Range.Delete()
            target_doc.ActiveWindow.Selection.SetRange(start_pos, end_pos)
            target_doc.ActiveWindow.Selection.Delete()
            
            print("Berhasil menghapus isi BAB 4 via Selection.")
        else:
